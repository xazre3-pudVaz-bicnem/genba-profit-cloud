...
import { dashboardStats, invoiceIsOverdue, monthlySeries, projectFinance } from "@/lib/calc";
import { DOCUMENT_STATUSES, DOCUMENT_TYPES, INVOICE_STATUSES, OVERDUE_BADGE } from "@/lib/constants";
import { daysUntil, longDate, pct1, relativeDate, shortDate, yen } from "@/lib/format";
          ...
          {unbilledRevenues.map((r) => {
            ...
            return (
              <PanelRow key={r.id} href={`/app/projects/${r.projectId}?tab=revenue`}>
                ...
                <Money value={r.amount} className="text-xs" />
              </PanelRow>
            );
          })}
        </Panel>
            ...
            return (
              <PanelRow key={inv.id} href={`/app/invoices/${inv.id}`}>
                ...
                <div className="flex shrink-0 flex-col items-end gap-1">
                  <Money value={inv.total} className="text-xs" />
                  <StatusBadge
                    meta={
                      overdue
